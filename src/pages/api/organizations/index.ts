...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.third_party_user?.length > 0) {
      const create_third_party_user = body.third_party_user;
      body.third_party_user = {
        create: create_third_party_user,
      };
    } else {
      delete body.third_party_user;
    }
